  ...
  return (
    <div style={{ marginBottom: 9 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 3 }}>
        ...
        <span><b style={{ color, fontSize: 12.5 }}>{fmt(cur)}{unit}</b> <span style={{ color: '#cbd5e1' }}>→ {fmt(end)}{unit}</span></span>
      </div>
...
export default function Hud({ ui, banner, stats, year, playing, onYear, onTogglePlay }) {
  return (
    <>
      <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 glass rounded-3xl px-5 py-2 flex items-center gap-4">
        <div className="font-round" style={{ fontSize: 15, color: '#334155' }}>⚡ 대한민국 전력계통</div>
        ...
        <div className="flex items-center gap-1 rounded-full px-3 py-1" style={{ background: 'rgba(255,255,255,0.7)', border: '1.5px solid rgba(148,163,184,0.3)' }}>
          <span>{WEATHER[ui.weather].icon}</span><span className="font-round" style={{ color: '#64748b', fontSize: 12 }}>{WEATHER[ui.weather].name}</span>
        </div>
      </div>

      {banner && (
        <div className="pop absolute top-20 left-1/2 -translate-x-1/2 z-20 rounded-full px-6 py-2 font-round" style={{ background: 'rgba(255,255,255,0.96)', border: '2px solid #fbbf24', color: '#b45309', boxShadow: '0 12px 26px rgba(251,191,36,0.28)', maxWidth: '74vw', textAlign: 'center' }}>
          {banner}
        </div>
      )}

      <div className="absolute top-3 left-3 z-20 w-[330px] max-w-[88vw] flex flex-col gap-3 max-h-[calc(100vh-120px)] overflow-y-auto">
        <div style={{ height: 52 }} />
      ...
      <div className="glass" style={{ position: 'absolute', bottom: 14, left: '50%', transform: 'translateX(-50%)', zIndex: 20, borderRadius: 20, padding: '10px 18px', width: 'min(560px, 92vw)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={onTogglePlay} className="font-round" style={{ border: 'none', cursor: 'pointer', borderRadius: 14, padding: '8px 14px', color: '#fff', background: playing ? 'linear-gradient(135deg,#fda4af,#fb7185)' : 'linear-gradient(135deg,#6ee7b7,#34d399)', boxShadow: '0 6px 14px rgba(120,150,190,0.25)' }}>
            {playing ? '⏸ 일시정지' : '▶ 재생'}
          </button>
          <div style={{ flex: 1 }}>
          ...
          <div className="font-round" style={{ fontSize: 22, color: '#0ea5e9', minWidth: 56, textAlign: 'right' }}>{Math.floor(year)}</div>
        </div>
